...
register_nav_menus( array(
	'primary' => __('Main Navigation', 'tpl_024_rwd'),
	));
...
add_filter( 'wp_list_categories', 'tpl_024_rwd_list_categories', 10, 2 );
function tpl_024_rwd_list_categories( $output, $args ) {
  $output = preg_replace('/<\/a>\s*\((\d+)\)/',' ($1)</a>',$output);
  return $output;
}

add_filter( 'get_archives_link', 'tpl_024_rwd_archives_link' );
function tpl_024_rwd_archives_link( $output ) {
...
/*	Add admin CSS
/*---------------------------------------------------------*/
function tpl_024_rwd_admin_css(){
	$adminCssPath = get_template_directory_uri().'/cTpl_admin.css';
	wp_enqueue_style( 'theme', $adminCssPath , false, '2012');
}
add_action('admin_head', 'tpl_024_rwd_admin_css', 11);


/*	Display navigation to next/previous pages when applicable
/*---------------------------------------------------------*/
function tpl_024_rwd_content_nav( $nav_id ) {
	global $wp_query;
